...
for (const line of lines) {
  // If it doesn't start with a number (followed by a dot) or space, it might be a date.
  // Exception: "Monday"
  if (line.match(/^\d+\./)) {
    // it's a student
    ...
    if (match && currentDate) {
      ...
      name = name.replace(/\d+\s*LVL$/i, '').trim();
      data.push({
        date: currentDate,
        name: name
      });
    ...
    // try to parse date
    let dateLine = line.toLowerCase().replace(/monday/, '9th march').trim();
    const match = dateLine.match(/(\d+)(?:st|nd|rd|th)?\s+(january|february|march|april|may|june|july|august|september|october|november|december)(?:\s+(\d{4}))?/i);
    if (match) {
...
const tsContent = `export const historicAttendance = ${JSON.stringify(data, null, 2)};\n`;
fs.writeFileSync('src/lib/historicData.ts', tsContent);
console.log('Done parsing ' + data.length + ' records.');
